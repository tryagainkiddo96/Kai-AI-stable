...
class KaiWatcher:
    ...
    def start(self):
        ...
        with self._lock:
            ...
            self._threads = []
            self._emit("watcher", "Proactive watcher online.", speak=False)
            ...
            for watcher_fn in watchers:
                t = threading.Thread(target=watcher_fn, args=(generation,), daemon=True)
                t.start()
    ...
    def stop(self):
        ...
        with self._lock:
            if self._running:
                self._emit("watcher", "Proactive watcher paused.", speak=False)
            self._running = False
    ...
    def status_snapshot(self) -> dict:
        ...
        return {
            ...
            "uptime_seconds": int(now - self._started_at) if self._running and self._started_at else 0,
    ...
    def _watch_time_of_day(self, generation: int):
        ...
        while self._is_active_generation(generation):
            ...
            if self._wait_or_stop(300, generation):
                return

    # ─── Idle Time ───

    def _watch_idle_time(self, generation: int):
        """Notice when the user has been idle."""
        idle_threshold = 600  # 10 minutes
        last_check = time.time()

        while self._is_active_generation(generation):
            if self._wait_or_stop(60, generation):
                return
    ...
    def _watch_downloads(self, generation: int):
        ...
        while self._is_active_generation(generation):
            if self._wait_or_stop(15, generation):
                return
            try:
    ...
    def _watch_wifi(self, generation: int):
        """Detect WiFi network changes."""
        # Import lazily to avoid circular
        if self._wait_or_stop(10, generation):
            return
        ...
        while self._is_active_generation(generation):
            ...
            if self._wait_or_stop(30, generation):
                return

    # ─── Battery ───

    def _watch_battery(self, generation: int):
        """Monitor battery level."""
        while self._is_active_generation(generation):
            if self._wait_or_stop(120, generation):
                return
            try:
    ...
    def _watch_clipboard(self, generation: int):
        """Watch clipboard for changes."""
        while self._is_active_generation(generation):
            if self._wait_or_stop(3, generation):
                return
            try:
    ...
    def _watch_social_timing(self, generation: int):
        """
    ...
    def _is_active_generation(self, generation: int) -> bool:
        return self._running and not self._stop_event.is_set() and generation == self._generation

    def _wait_or_stop(self, seconds: float, generation: int) -> bool:
        self._stop_event.wait(seconds)
        return not self._is_active_generation(generation)
    ...
    def _deliver_proactive_via_chat(self, message: str) -> None:
        ...
        try:
            ...
            if hasattr(self.assistant, '_invalidate_context_cache'):
                self.assistant._invalidate_context_cache()
