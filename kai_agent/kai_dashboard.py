...
def _wait_for_port(host: str, port: int, seconds: int = 20) -> bool:
...
def _start_ollama_if_needed() -> None:
    ...
    try:
        ...
        if _wait_for_port(OLLAMA_HOST, OLLAMA_PORT, seconds=20):
            console.print("[dim]Ollama started successfully.[/dim]")
        else:
            console.print("[yellow]Ollama did not become ready. You may need to start it manually.[/yellow]")
    except Exception:
        pass
...
def main():
    ...
    if args.provider in (None, "ollama"):
    ...
    while running:
        ...
        # Instant responses for common requests (skip LLM)
        if any(w in lower for w in ["list your skills", "list your capabilities", "what can you do", "your capabilities", "your skills"]):
            print_message("user", user_input)
            cap = (
                "My capabilities:\n\n"
                "• Chat & Q&A — general conversation and questions\n"
                "• Web — browse, search, take screenshots\n"
                "• File operations — read, write, organize files\n"
                "• Shell commands — run system commands\n"
                "• Memory — store and recall information\n"
                "• Mood & emotions — emotional responses\n"
                "• Skills system — learn new skills over time\n"
                "• Hardware integration — interact with hardware\n"
                "• Kali tools — pentesting (when available)\n\n"
                "Type /menu for the full command list."
            )
            print_message("kai", cap)
            continue
