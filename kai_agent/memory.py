...
def _safe_load_json(path: Path, default):
    try:
        ...
        backup = path.with_suffix(path.suffix + f".bad-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}")
        try:
            path.rename(backup)
        except Exception:
            pass
...
class KaiMemory:
    ...
    def forget_note(self, query: str) -> dict:
        ...
        for note in notes:
            content = str(note.get("content", "")).lower() if isinstance(note, dict) else str(note).lower()
            if query in content:
                removed.append(note)
            else:
                kept.append(note)
        ...
        if not removed:
            return {"ok": False, "error": f"No memory note matched: {query}"}

        _atomic_write_text(self.notes_path, json.dumps(kept, indent=2))
        ...
        if notes:
            ...
            for n in notes:
                if isinstance(n, dict):
                    note_texts.append(f"- {n.get('content', n.get('text', str(n)))}")
                else:
        ...
        if tasks:
            ...
            for t in tasks:
                if isinstance(t, dict):
                    task_texts.append(f"- [{t.get('status', '?')}] {t.get('title', t.get('text', str(t)))}")
                else:
